...
class MailsController extends NewsletterAppController {
 ...
 function beforeFilter() {
  ...
  function show() {
    $this->Mail->order = 'created DESC'; // simulating the model having a default order
    $this->set('mail', $this->Mail->find());
    $this->layout = 'clean';
  }

  function admin_add() {
      if(!empty($this->request->data)) {
          $this->request->data['Mail']['read_confirmation_code'] = md5(date('Y-m-d H:i:s').$this->request->data['Mail']['subject']);
  ...
  function admin_send($id) {
    ...
    $rest = $this->GroupSubscription->restingSubscriptions($id, array('count' => true, 'limit' => $limit));
  ...
  **/
  function admin_send_mail($id) {
    ...
    if(!empty($subscriptions)) {
	  ...
	  $destinatari = $this->extractEmailAndName($subscriptions);
      $this->sendEmail($mail['Mail']['subject'], $mailToSend, $destinatari, $mail['Mail']['from_email'], $mail['Mail']['from']);
	  $this->log($destinatari, 'debug');
