...
class SubscriptionsController extends NewsletterAppController {
  ...
  function sendit(){
      ...
      $this->sendEmail('ciao', 'come stai?', 'lastexxit@gmail.com');
  }
 ...
 function beforeFilter() {
  ...
  function unsubscribe() {
    if($this->isNotEmpty('Subscription.email')) {
      $subscribed = $this->Subscription->find('first', array('conditions' => array('email' => $this->request->data['Subscription']['email'], 'opt_out_date' => null)));
      if($subscribed) {
        ...
        //invece che disattivare l'utente, aggiungo al linea 45 il delte del record
        #send email
        ...
        $this->sendEmail($subject, 'You have successfully unsubscribed to our newsletter', $subscription['Subscription']['email']);
  ...
  function subscribe() {
    if(!empty($this->request->data) && (!empty($this->request->data['Subscription']['email']))) {
      $subscribed = $this->Subscription->findByEmail($this->request->data['Subscription']['email']);
      ...
      if(empty($subscribed) || 
        ...
        $confirmation_code = md5(date('Y-m-d H:i:s').$this->request->data['Subscription']['name'].$this->request->data['Subscription']['email']);
        
        if(!empty($subscribed)) {
          $this->request->data['Subscription']['id'] = $subscribed['Subscription']['id'];
        }
        
        $this->request->data['Subscription']['confirmation_code'] = $confirmation_code;
        ...
        //metto a 2 il valore di default del gruppo (friends)
        if(!$site_group) {$site_group = '2';}
        ...
        $confirm_url = Configure::read('Newsletter.confirm_url');
        $this->set('url', $confirm_url);
        $message_confirm_subscriptions = "click here to confirm your subscription <a href = \"".$confirm_url.$subscription['Subscription']['confirmation_code']."\">";
	$message_confirm_subscriptions.=$confirm_url.$subscription['Subscription']['confirmation_code']."</a>";
        $this->sendEmail($subject, $message_confirm_subscriptions, $subscription['Subscription']['email']);
  ...
  function admin_index() {
    ...
    if(!empty($this->request->data['Filter']['value'])) {
      $filter = $this->request->data['Filter']['value'];
  ...
  function admin_import_csv() {
    if (!empty($this->request->data) && is_uploaded_file($this->request->data['Subscription']['csv']['tmp_name'])) {
	    set_time_limit(0);
	    
	    $lines = $this->readUploadedCSV($this->request->data['Subscription']['csv']['tmp_name']);
		  $errors = array();
		  $data = array();
		  ...
		  $this->Subscription->importCsv($data, $this->request->data['Group']['Group']);
