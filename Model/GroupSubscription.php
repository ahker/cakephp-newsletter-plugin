...
class GroupSubscription extends NewsletterAppModel { 
  ...
  **/
  function restingSubscriptions($mail_id, $options=array()) {
    ...
    $result = $this->find('all', array('fields' => $fields, 'conditions' => array('newsletter_group_id' => $groups, 'Subscription.id >' => $last_sent, 'Subscription.opt_out_date' => null), 'order' => 'Subscription.id', 'limit' => $limit));
